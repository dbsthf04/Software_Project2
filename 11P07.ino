...
#define _DUTY_MIN 470 // servo full clockwise position (0 degree)
...
void loop() {
  ...
  } else if(360.0 <= dist_ema){
    myservo.writeMicroseconds(_DUTY_MAX);
  } else{
    myservo.writeMicroseconds(_DUTY_MIN + (dist_ema - 180.0) * (_DUTY_MAX - _DUTY_MIN) / 180.0);
  }
